...
use App\Notifications\WhatchNotification;
...
class AttentionFormController extends Controller
{
    ...
    public function sendEmail(Request $request, AttentionForm $attentionForm)
    {
        ...
        $name_plaintiff = $attentionForm->name_plaintiff;
        $email_plaintiff = $attentionForm->email_plaintiff;
        $whatch = $request->whatch;

        Mail::to($email_plaintiff)->send(new AttentionMail($name_plaintiff, $whatch));

        // Notification::route('mail', $email_plaintiff)->notify(new WhatchNotification($name_plaintiff, $whatch));
